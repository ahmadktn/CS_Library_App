            ...
            try {
                const infos = await Promise.all(
                    documents.map(async (doc: any, index: number) => {
                        try {
                            if (!doc.fileSource) {
                                console.warn(`Document ${index} in ${courseCode} missing fileSource`);
                                return {
    ...
    const handleDownload = async (doc: DocumentInfo) => {
        ...
        try {
            ...
            if (Platform.OS === 'android') {
                ...
                // Copy file to cache first with correct filename
                const tempPath = `${FileSystem.cacheDirectory}${doc.name}`;
                ...
                Alert.alert('Success', `"${doc.name}" has been saved successfully.`);
            } else {
                // iOS: share sheet with "Save to Files" is the native equivalent
